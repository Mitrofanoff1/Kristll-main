...
export default function Results({ onOpenModal }: { onOpenModal?: (type?: any) => void }) {
  ...
  const slides = [
    ...
    { id: 2, area: "Голени", result: "Результат после 4 сеансов", before: "/result-2-before.webp", after: "/result-2-after.webp" },
    { id: 3, area: "Глубокое бикини", result: "Результат после 5 сеансов", before: "/result-3-before.webp", after: "/result-3-after.webp" },
    { id: 4, area: "Подмышечные впадины", result: "Результат после 4 сеансов", before: "/result-4-before.webp", after: "/result-4-after.webp" },
  ];
  ...
  return (
    <section id="results" className="py-12 md:py-24 bg-[#FAFAFA] overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-10 md:mb-16">
          <h2 className="text-2xl md:text-5xl font-black uppercase mb-4 tracking-tighter">Результаты наших клиентов</h2>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <button onClick={(e) => { e.stopPropagation(); prevSlide(); }} className="absolute left-[-10px] md:left-[-30px] top-[45%] md:top-1/2 -translate-y-1/2 w-9 h-9 md:w-14 md:h-14 bg-white rounded-full flex items-center justify-center shadow-xl z-20 border border-gray-100"><ChevronLeft className="w-5 h-5 text-black" /></button>
          <button onClick={(e) => { e.stopPropagation(); nextSlide(); }} className="absolute right-[-10px] md:right-[-30px] top-[45%] md:top-1/2 -translate-y-1/2 w-9 h-9 md:w-14 md:h-14 bg-white rounded-full flex items-center justify-center shadow-xl z-20 border border-gray-100"><ChevronRight className="w-5 h-5 text-black" /></button>
          <div className="flex justify-center gap-2 md:gap-3 mt-6">
            {slides.map((_, i) => <button key={i} onClick={() => setCurrentSlide(i)} className={`h-1 md:h-1.5 rounded-full transition-all ${i === currentSlide ? 'w-8 md:w-10 bg-accent' : 'w-2 md:w-3 bg-gray-200'}`} />)}
          </div>
        </div>
