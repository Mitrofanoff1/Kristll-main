...
export default function Block3({ onOpenModal }: { onOpenModal: () => void }) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      diff > 0 ? handleManualNext() : handleManualPrev();
    }
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-20">
          <div className="w-full lg:w-1/2">
            ...
            >
              ...
              <button onClick={handleManualPrev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-md z-30 active:scale-90 text-black"><ChevronLeft size={24} /></button>
              <button onClick={handleManualNext} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-md z-30 active:scale-90 text-black"><ChevronRight size={24} /></button>
            </div>
